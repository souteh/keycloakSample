...
@Component({
  selector: 'jhi-actionparam',
  templateUrl: './actionparam.component.html',
})
export class ActionparamComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.actionparamService
      ...
      .subscribe(
        (res: HttpResponse<IActionparam[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  protected onSuccess(data: IActionparam[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/actionparam'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
