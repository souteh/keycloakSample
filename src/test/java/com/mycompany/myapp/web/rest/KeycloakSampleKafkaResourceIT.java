...
class KeycloakSampleKafkaResourceIT {
    ...
    void setup() {
        ...
        KeycloakSampleKafkaResource kafkaResource = new KeycloakSampleKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/keycloak-sample-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/keycloak-sample-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
